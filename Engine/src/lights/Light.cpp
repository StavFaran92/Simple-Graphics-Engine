...
Light::Light()
	:color(glm::vec3(1.0f, 1.0f, 1.0f)), m_ambientIntensity(0.2f), m_diffuseIntensity(.5f)
{}

Light::Light(glm::vec3 color, float aIntensity, float dIntensity) :
	color(color), m_ambientIntensity(aIntensity), m_diffuseIntensity(dIntensity)
{}

void Light::useLight(Shader& shader, int index)
{
	shader.setUniformValue(m_name + "[" +std::to_string(index) + "]" + ".color", color);
