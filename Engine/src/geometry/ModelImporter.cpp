...
void extractAiMaterialProperties(const aiMaterial* aiMat, MaterialData& materialData)
{
	...
	if (aiMat->Get(AI_MATKEY_OPACITY, opacityFactor) == aiReturn_SUCCESS)
	{
		if (opacityFactor < 1.f)
		{
			...
			materialData.setProperty(SHADER_PROPERTY_PBR_OPACITY_FACTOR, opacityFactor);
		}
	...
	{
		materialData.setProperty(SHADER_PROPERTY_PBR_COLOR_DIFFUSE, glm::vec3(diffuseColor.r, diffuseColor.g, diffuseColor.b));
	}

	ai_real rounghnessFactor;
	if (aiMat->Get(AI_MATKEY_ROUGHNESS_FACTOR, rounghnessFactor) == aiReturn_SUCCESS)
	{
		materialData.setProperty(SHADER_PROPERTY_PBR_ROUGHNESS_FACTOR, rounghnessFactor);
	}

	ai_real metallicFactor;
	if (aiMat->Get(AI_MATKEY_METALLIC_FACTOR, metallicFactor) == aiReturn_SUCCESS)
	{
		materialData.setProperty(SHADER_PROPERTY_PBR_METALLIC_FACTOR, metallicFactor);
	}
