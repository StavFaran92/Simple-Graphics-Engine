...
bool MeshGroupAssetManager::copyFiles(const std::string& fileLocation, AssetInfo& aInfo)
{
	return Engine::get()->getSubSystem<ModelImporter>()->copyFiles(fileLocation, aInfo);
}

ResourceWrapper<Resource> MeshGroupAssetManager::load(AssetInfo& aInfo)
{
...
std::map<int, AssetWrapper<Material>> MeshGroupAssetManager::getLoadedMaterials()
{
	//return m_lastLoadedModelInfo.materials;
	return Engine::get()->getSubSystem<ModelImporter>()->getLastImportedMaterial().materials;
}

void MeshGroupAssetManager::save(Asset mesh, const AssetInfo& aInfo)
{
	MeshExporter::exportMesh(aInfo, MeshGroupAsset(mesh));
}
...
std::map<int, AssetWrapper<Material>> MeshGroup::getLastLoadedMaterials()
{
	return Engine::get()->getSubSystem<ModelImporter>()->getLastImportedMaterial().materials;
}
